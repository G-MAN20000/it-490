...
export class Sherming extends Phaser.Physics.Arcade.Sprite {
    ...
    public initBody() {
        ...
        body.setSize(this.width, this.height - 2, true);
        this.setDisplaySize(this.width, this.height - 2);
    ...
    public isOnGround(): boolean {
        const body = this.body as Phaser.Physics.Arcade.Body;
        return body != null && (body.blocked.down || body.touching.down);
    }
    ...
    public static createWithDirection(scene: Phaser.Scene, x: number, y: number, scale: number, startingDirection: Direction, textureKey: string | null, frameKey: string | null): Sherming {
        const sherming = new Sherming(scene, x, y, scale, startingDirection);
        if (textureKey != null && frameKey != null) sherming.setTexture(textureKey, frameKey);
        scene.physics.add.existing(sherming);
        sherming.initBody();
        return sherming;
