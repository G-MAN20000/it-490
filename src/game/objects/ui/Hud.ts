...
export class HUDScene extends Phaser.Scene {
    ...
    private lastLevelTime: number = -1;
    ...
    create() {
        ...
        this.add.rectangle(0, h - HUDScene.HUD_HEIGHT, w, HUDScene.HUD_HEIGHT, 0x00000000, 0.9)
            .setOrigin(0, 0)
            .setScrollFactor(0);
    ...
    update(_time: number, delta: number) {
        if (this.levelTimeText != null) {

            this._levelTime -= delta / 1000;
            if (this._levelTime < 0) {
                this._levelTime = 0;
            }

            const currentSeconds = Math.floor(this._levelTime);
            const lastSeconds = Math.floor(this.lastLevelTime);

            if (currentSeconds != lastSeconds) {
                const minutes = Math.floor(this._levelTime / 60);
                const seconds = Math.floor(this._levelTime % 60);
                this.levelTimeText.setText('Time: ' + `${minutes}:${seconds.toString().padStart(2, '0')}`);
                this.lastLevelTime = this._levelTime;
            }
        }
    }
    ...
    public setRemaining(value: number) {
        ...
        this.remainingText.setText('Shermings Left: ' + this._remaining);
    }

    public setNeeded(value: number) {
        this._needed = value;
        this.neededText.setText('Needed to Pass: ' + this._needed);
    }
